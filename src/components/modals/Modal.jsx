...
function ModalWindow({ children }) {
  ...
  return (
    <Card className={`custom-modal-window ${transitionClass}`}>{children}</Card>
  );
}
  ...
  return (
    <CardHeader>
      ...
      <div className={`close-menu d-flex align-items-center`}>
        <ModalCloseButton color={`dark`} onClose={onClose} />
      </div>
    </CardHeader>
  );
}
...
function ModalWindowTransparent({ children, onClose }) {
  return (
    <div className={`custom-modal-window-transparent`}>
      ...
      <ModalCloseButton color={`highlight`} onClose={onClose} />
    </div>
  );
}
