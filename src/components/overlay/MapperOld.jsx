...
import { processAICoordinates, screenToPdfCoords, pdfToScreenCoords, snapToGrid, rectPxToPt, rectPtToPx } from "../../utils/pdfCoords";
...
export default function Mapper() {
  ...
  const handleDragEnd = (event) => {
    ...
    if (over && over.id === 'pdf-canvas') {
      ...
      if (aiField) {
        ...
        const dropX = event.delta.x + canvasRect.left;
        const dropY = event.delta.y + canvasRect.top;
        ...
        const newField = {
          ...
          type: aiField.type,
          rect: [
            pdfCoords.x - aiField.width / 2,
            pdfCoords.y - aiField.height / 2,
            pdfCoords.x + aiField.width / 2,
            pdfCoords.y + aiField.height / 2
          ],
  ...
  // Draggable Field Tray Item Component
  ...
  // Droppable PDF Canvas Component
  ...
  const ResizableFieldBox = ({ field, isSelected, onUpdate, onSelect }) => {
    ...
    const handleDragStop = (e, d) => {
      const newRect = [
        snapToGrid(rectPxToPt([d.x, d.y, d.x + width, d.y + height])[0]),
        snapToGrid(rectPxToPt([d.x, d.y, d.x + width, d.y + height])[1]),
        snapToGrid(rectPxToPt([d.x, d.y, d.x + width, d.y + height])[2]),
        snapToGrid(rectPxToPt([d.x, d.y, d.x + width, d.y + height])[3])
      ];
