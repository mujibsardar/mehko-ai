...
test.describe('Application Steps', () => {
          ...
          documents: [
            {
              ...
              _fields: {
                ...
                _description: { stringValue: 'Home-based restaurant permit for up to 30 meals/day or 90 meals/week, max $100,000 annual sales (adjusted). No signage; food must be prepared, cooked, and served/delivered the same day.' },
                    ...
                    values: [
                      {
                        mapValue: {
                          fields: {
                            ...
                            _title: { stringValue: 'Plan Your MEHKO' },
                            _type: { stringValue: 'info' },
                            _content: { stringValue: 'Start your MEHKO journey with planning resources and guides.' },
                            _action_required: { booleanValue: false }
                          }
                        }
                      },
                      {
                        _mapValue: {
                          fields: {
                            ...
                            _title: { stringValue: 'Approvals & Training' },
                            _type: { stringValue: 'info' },
                            _content: { stringValue: 'Complete required training and obtain necessary approvals.' },
                            _action_required: { booleanValue: true }
                          }
                        }
                      },
                      {
                        _mapValue: {
                          fields: {
                            ...
                            _title: { stringValue: 'Standard Operating Procedures (SOP)' },
                            _type: { stringValue: 'pdf' },
                            _content: { stringValue: 'Download and complete the SOP form.' },
                            _action_required: { booleanValue: true },
                            _fill_pdf: { booleanValue: true },
                            _formId: { stringValue: 'mehkosop.pdf' },
                            _appId: { stringValue: 'san_diego_county_mehko' }
                          }
                        }
                      },
                      {
                        _mapValue: {
                          fields: {
                            ...
                            _title: { stringValue: 'Health Permit Application' },
                            _type: { stringValue: 'pdf' },
                            _content: { stringValue: 'Complete the main permit application form.' },
                            _action_required: { booleanValue: true },
                            _fill_pdf: { booleanValue: true },
                            _formId: { stringValue: 'publications_permitapp152.pdf' },
                            _appId: { stringValue: 'san_diego_county_mehko' }
                          }
                        }
                      }
  ...
  test('should display application steps correctly', async ({ page }) => {
    ...
    await expect(page.locator('.step-_item: has-text("Approvals & Training")')).toBeVisible();
    await expect(page.locator('.step-_item: has-text("Standard Operating Procedures (SOP)")')).toBeVisible();
    await expect(page.locator('.step-_item: has-text("Health Permit Application")')).toBeVisible();
  });

  test('should show step completion status', async ({ page }) => {
    // Verify completed step has completion indicator
    const completedStep = page.locator('.step-_item: has-text("Plan Your MEHKO") .checkmark');
    ...
    const incompleteStep = page.locator('.step-_item: has-text("Approvals & Training") .checkmark');
  ...
  test('should handle step selection and content display', async ({ page }) => {
    ...
    await page.click('.step-_item: has-text("Plan Your MEHKO")');
    
    // Verify step content is displayed using specific selector to avoid strict mode
    await expect(page.locator('.main-content')).toBeVisible();
    await expect(page.locator('_h2: has-text("Plan Your MEHKO")')).toBeVisible();
  });

  test('should handle PDF step content', async ({ page }) => {
    // Click on SOP form step
    await page.click('.step-_item: has-text("Standard Operating Procedures (SOP)")');
    
    // Verify PDF step content is displayed
    await expect(page.locator('.main-content')).toBeVisible();
    
    // Look for download button or PDF viewer
    const downloadButton = page.locator('_button: has-text("Download"), _button: has-text("View PDF"), .pdf-viewer');
  ...
  test('should handle step completion', async ({ page }) => {
    ...
    await page.click('.step-_item: has-text("Approvals & Training")');
    
    // Look for mark complete button
    const completeButton = page.locator('_button: has-text("Mark Complete"), _button: has-text("Complete")');
    if (await completeButton.isVisible()) {
      ...
      const completedIndicator = page.locator('.step-_item: has-text("Approvals & Training") .checkmark');
  ...
  test('should handle step navigation and content persistence', async ({ page }) => {
    ...
    await page.click('.step-_item: has-text("Approvals & Training")');
    await expect(page.locator('.main-content')).toBeVisible();
    await expect(page.locator('_h2: has-text("Approvals & Training")')).toBeVisible();
  ...
  test('should display progress information', async ({ page }) => {
    ...
    if (await progressBar.isVisible()) {
      ...
      const progressText = page.locator('.progress-info _small: has-text("of")');
  ...
  test('should show action required indicators', async ({ page }) => {
    ...
    const planningStep = page.locator('.step-_item: has-text("Plan Your MEHKO")');
    await expect(planningStep).toBeVisible();
    
    // Verify other steps show action required
    await expect(page.locator('.step-_item: has-text("Approvals & Training")')).toBeVisible();
    await expect(page.locator('.step-_item: has-text("Prepare Required Documents")')).toBeVisible();
  });
  ...
  test('should handle step completion workflow', async ({ page }) => {
    ...
    if (await completeButton.isVisible()) {
      ...
      await expect(page.locator('.step-_item: has-text("Plan Your MEHKO")')).toBeVisible();
    }
